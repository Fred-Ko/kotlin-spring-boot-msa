...
data class User(
    ...
    val password: Password, // encoded password
    val email: Email,
    val name: Name,
    ...
    fun addAddress(addressData: Address): User {
        ...
        if (addresses.any {
                it.addressId == addressData.addressId
            }
        ) {
    ...
    fun updateAddress(
        ...
        if (status == UserStatus.WITHDRAWN) throw UserDomainException.User.AlreadyWithdrawn()
        if (addressId != updatedAddressData.addressId) {
            throw UserDomainException.Address.IdMismatch(
                addressId.value.toString(),
                updatedAddressData.addressId.value.toString(),
            )
        }
